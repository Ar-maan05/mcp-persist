"""
examples/postgres_server.py
===========================
Minimal MCP server using PostgresEventStore for SSE resumability.

Ideal for: deployments already running PostgreSQL that want durable SSE
resumability across restarts — including multi-node / team setups. Events are
persisted to Postgres via an asyncpg connection pool.
...
# ---------------------------------------------------------------------------
# ASGI app with PostgresEventStore
# ---------------------------------------------------------------------------

POSTGRES_URL = os.environ.get("DATABASE_URL", "postgresql://postgres@localhost:5432/postgres")
...
async def lifespan(app: Starlette) -> AsyncIterator[None]:
    ...
    try:
        ...
        async with session_manager.run():
            logging.info("Server ready — Postgres event store at %s", POSTGRES_URL)
            yield
