...
@pytest.mark.anyio
async def test_replay_skips_corrupt_payload_without_aborting_stream(store, redis_client, caplog):
    ...
    await redis_client.hset(f"test:event:{bad}", "payload", "{not valid json")
